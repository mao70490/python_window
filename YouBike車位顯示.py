...
class Window(tk.Tk):
    def __init__(self):
        ...
        #建立message介面
        messageDisplayFrame = tk.Frame(self,height=100)
        self.mdayLabel = tk.Label(messageDisplayFrame, text="紀錄時間:")
    ...
    def userChoicedRadioButton(self):
        ...
        self.mdayLabel["text"] = "紀錄時間:%d年%d月%d日  %d:%d:%d" % (structTime.tm_year,structTime.tm_mon,structTime.tm_mday,structTime.tm_hour,structTime.tm_min,structTime.tm_sec)
        self.snaLabel["text"] = "站名:%s" % infomation["sna"]
        self.arLabel.configure(text="地址:{0:s}".format(infomation['ar']))
        self.bempLabel["text"] = "總空位數量:%d" % infomation["bemp"]
        self.sbiLabel["text"] = "可借車數:%d" % infomation["sbi"]
        self.totLabel["text"] = "總車數:%d" % infomation["tot"]
